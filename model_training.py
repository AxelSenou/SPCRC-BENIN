...
df_train_raw = df_brut.iloc[train_idx].copy()
df_test_raw  = df_brut.iloc[test_idx].copy()
...
# Sauvegarder la moyenne nationale de Temp_Max pour l'inférence
...
with open(config.OUTPUT_DIR / config.FEATURES_PATH, "w", encoding="utf-8") as f:
    f.write(",".join(features_sel))

with open(config.OUTPUT_DIR / "communes_connues.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(communes_train))

# Statistiques globales pour l'inférence (inclut mean_temp_max pour Dev_Thermique)
global_stats = {
...
axes[1].hist(residus, bins=25, color="steelblue", edgecolor="white", alpha=0.8)
axes[1].axvline(0, color='red', linestyle='--', lw=2)
axes[1].axvline(residus.mean(), color='orange', linestyle='--', lw=1.5,
